...
$query = $bdd->prepare("SELECT nbligne_objectif, nbheure_objectif FROM client_cat_synthese_fiche_obj WHERE debut_objectf <= '".date("Y-m-d")."' AND fin_objectif >= '".date("Y-m-d")."' ORDER BY id_objectif DESC LIMIT 0, 1");
$query->execute();
$donnees = $query->fetch();
...
<head>
  ...
  <title><?php echo $titre_dossier; ?></title>
...
<link rel="stylesheet" href="css/layout_acide.css">
...
?>
	...
	<div class="page-content">
        ...
        <div class="container-fluid">     
          <h2 class="content-heading">Dossiers <?php echo $titre_dossier; ?> : <b>Partie SOCIÉTÉ</b></h2>
          <div class="content-description">Heures : <span class="badge badge-bittersweet badge-rounded"><?php echo $heure;?> H</span> / Fiches : <span class="badge badge-bittersweet badge-rounded"><?php echo $ligne;?> F</span></div>
          <div class="row">
            <div class="col-xl-3 col-lg-3 col-md-3">
              <div class="widget widget-alpha widget-alpha--color-green-jungle">
                <div>
					...
					if($_GET['idcatt'] == 2)	{
					$query = $bdd->prepare("SELECT SEC_TO_TIME(SUM(temps_sec)+540000) AS datee FROM client_cat_synthese_fiche_update WHERE lot_id IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga)");
					}else{
					$query = $bdd->prepare("SELECT SEC_TO_TIME(SUM(temps_sec)) AS datee FROM client_cat_synthese_fiche_update WHERE lot_id IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga)");
					}
            ...
            <div class="col-xl-3 col-lg-3 col-md-3">
              <div class="widget widget-alpha widget-alpha--color-green-jungle">
                <div>
                    <div class="widget-alpha__amount">
                    <?php 
					$query = $bdd->prepare("SELECT COUNT(DISTINCT(siret_client)) FROM `client_traitement` WHERE id_cat IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga) AND reporting <> 0");
		$query->bindParam(":id_client_cat_oraga", $_GET['idcatt'], PDO::PARAM_INT);
		$query->execute();
		$somme_traite = $query->fetchColumn();
		$query->closeCursor();	
                    echo $somme_traite; 
                    ?></div>
                  <div class="widget-alpha__description">Traité</div>
                </div>
                <span class="widget-alpha__icon iconfont iconfont-info"></span>
              </div>
            </div>
            <div class="col-xl-3 col-lg-3 col-md-3">
              <div class="widget widget-alpha widget-alpha--color-green-jungle">
                <div>
					...
					$query = $bdd->prepare("SELECT COUNT(DISTINCT(siret_client)) FROM `client_traitement` WHERE id_cat IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga)");
		$query->bindParam(":id_client_cat_oraga", $_GET['idcatt'], PDO::PARAM_INT);
		$query->execute();
		$somme_total = $query->fetchColumn();
		$query->closeCursor();	
                    echo $somme_traite; 
                    ?></div>
                  <div class="widget-alpha__description">Total Dédoublé</div>
                </div>
                <span class="widget-alpha__icon iconfont iconfont-info"></span>
              </div>
            </div>
          ...
          <div class="main-container">
            ...
            <div class="row">
            <div class="col-lg-6">
            <a class="btn btn-success icon-left btn-sm mr-3" href="ClientAjout-<?php echo $_GET['idcatt'];?>">Ajouter un dossier <span class="btn-icon iconfont iconfont-plus-v1"></span></a>
            <a class="btn btn-secondary icon-left btn-sm mr-3" href="ClientObj-<?php echo $_GET['idcatt'];?>">Objectifs société</span></a>
        	...
        	<div id="modal-settings" class="modal fade custom-modal-tabs">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <form action="ClientRech" method="post" name="filtre">
                          ...
                          <div class="col-lg-12">
                            <div class="form-group">
                              ...
                              <select class="form-control" data-placeholder="Tous les intervenants disponibles" name="collab">
                                <option></option>
                                <?php
                								/*$query = $bdd->prepare("SELECT operateur, user_id FROM `client_traitement` WHERE user_id <> 0 GROUP BY user_id");
                								$query->execute();	
                								while($query_collab = $query->fetch()){
                								 echo '<option value="'.$query_collab['user_id'].'">'.$query_collab['operateur'].'</option>';	
                								}
                								$query->closeCursor();*/						
                								?>
                              </select>
            ...
            <a class="btn btn-info icon-right btn-sm mr-3" href="Contact-<?php echo $_GET['idcatt'];?>" id="Contact">Aller à la Partie Contact <span class="btn-icon iconfont iconfont-step-arrow-right"></span></a>
            
            <a class="btn btn-danger icon-left btn-sm mr-3" href="ClientBiblioGlobale-<?php echo $_GET['idcatt'];?>">Téléchargement des Donnée globale</a>
            </div>
            <div class="col-lg-3">
            ...
            <table class="datatable table table-striped" id="table_cat_fichier_client" data-id="<?php echo $_GET['idcatt'];?>">
            <thead>
            <tr>
            <th>DOSSIER</th>
            <th>STATUT</th>
            <th>GLOBAL doublé</th>
            <th>GLOBAL dédoublé</th>
            <th>TRAITÉ</th>
            <th>EN %</th>
            <th>CC</th>
            <th>DÉBUT</th>
...
<div class="lightbox_container">
  ...
  <div class="lightbox_content">
            ...
            <div class="input_container">
            	<label for="reporting">Nom : <span class="required">*</span></label>
...
<script src="module/client/table/js/webapp_cat_fichier_client.js"></script>
