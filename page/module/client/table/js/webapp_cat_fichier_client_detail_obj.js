...
$(document).ready(function(){
  ...
  var table_companies = $('#table_gestion_traitement_lk_obj').dataTable({
	"bStateSave": true,
    "ajax": "module/client/table/php/data_cat_fichier_client.php?job=get_gestion_traitement_lk_obj",
    "columns": [
	  { "data": "ligne", "sClass": "" },
      { "data": "heure", "sClass": "" },
  ...
  $(document).on('submit', '#form_lk.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/client/table/php/data_cat_fichier_client.php?job=add_gestion_traitement_lk_obj',
        cache:        false,
        data:         form_data,
        dataType:     'json',
  ...
  $(document).on('click', '#function_edit_obj', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/client/table/php/data_cat_fichier_client.php?job=get_gestion_traitement_add_lk_obj',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('submit', '#form_lk.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_lk').serialize();
      var request   = $.ajax({
        url:          'module/client/table/php/data_cat_fichier_client.php?job=edit_gestion_traitement_lk_obj&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
			...
			buttons: {
				confirm: {
					...
					action: function() {
						...
						var request = $.ajax({
						url:          'module/client/table/php/data_cat_fichier_client.php?job=delete_gestion_traitement_lk_obj&id=' + id + '&cat=' + doc_up,
						cache:        false,
						dataType:     'json',
						contentType:  'application/json; charset=utf-8',
						type:         'get'
