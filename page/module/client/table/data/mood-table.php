...
$query = $bdd->prepare("SELECT user_name, DATE_FORMAT(date_debut_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsdebut, DATE_FORMAT(date_fin_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsfin, SEC_TO_TIME(temps_sec) AS traitement FROM client_cat_synthese_fiche_update WHERE fiche_id = :fiche_id ORDER BY fiche_up_id DESC");	
