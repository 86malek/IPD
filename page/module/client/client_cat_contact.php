...
$query = $bdd->prepare("SELECT nbligne_objectif, nbheure_objectif FROM client_cat_synthese_fiche_obj WHERE debut_objectf <= '".date("Y-m-d")."' AND fin_objectif >= '".date("Y-m-d")."' AND type = 1 ORDER BY id_objectif DESC LIMIT 0, 1");
$query->execute();
$donnees = $query->fetch();
...
?>
	...
	<div class="page-content">
        ...
        <div class="container-fluid">     
          <h2 class="content-heading">Dossiers <?php echo $titre_dossier; ?> : <b>Partie CONTACT</b></h2>
            ...
            <div class="col-xl-3 col-lg-3 col-md-3">
              <div class="widget widget-alpha widget-alpha--color-green-jungle">
                <div>
                    ...
                    <?php 
					if($_GET['idcatt'] == 2)	{
		$query = $bdd->prepare("SELECT SEC_TO_TIME(SUM(temps_sec) + 825000) AS datee FROM client_cat_synthese_fiche_update_contact WHERE lot_id IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga)");
		}else{
		$query = $bdd->prepare("SELECT SEC_TO_TIME(SUM(temps_sec)) AS datee FROM client_cat_synthese_fiche_update_contact WHERE lot_id IN (SELECT id_cat FROM client_cat WHERE id_client_cat_oraga = :id_client_cat_oraga)");
		}
		...
		$jh_lot = round($duree_decimal/8, 2);	
          ...
          <div class="main-container">
            ...
            <div class="row">
            <div class="col-lg-6">
            <a class="btn btn-info icon-left btn-sm mr-3" href="Client-<?php echo $_GET['idcatt'];?>" id="Société">Retour à la Partie Société <span class="btn-icon iconfont iconfont-step-arrow-left"></span></a> 
            <a class="btn btn-secondary icon-left btn-sm mr-3" href="ClientObjContact">Objectifs contact</span></a>
                          ...
                          <div class="col-lg-12">
                            <div class="form-group">
                              ...
                              <select class="form-control" data-placeholder="Tous les intervenants disponibles" name="collab">
                                <option></option>
                                <?php
                								/*$query = $bdd->prepare("SELECT operateur_contact, user_id_contact FROM `client_traitement` WHERE user_id_contact <> 0 GROUP BY user_id_contact");
                								$query->execute();	
                								while($query_collab = $query->fetch()){
                								 echo '<option value="'.$query_collab['user_id_contact'].'">'.$query_collab['operateur_contact'].'</option>';	
                								}
                								$query->closeCursor();*/						
                								?>
                              </select>
            ...
            <th>OK / En charge du Transport</th>
            <th>OK / Prise en charge externe</th>
            <th>KO</th>
            <th>EN COURS</th>
            <th>REFUS</th>
...
<script src="module/client/table/js/webapp_cat_fichier_client_contact.js"></script>
