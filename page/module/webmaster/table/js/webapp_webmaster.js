$(document).ready(function(){
  ...
  var table_companies = $('#table_webmaster').dataTable({
	  "bStateSave": true,
    "ajax": "module/webmaster/table/php/data_webmaster.php?job=get_webmaster",
    "columns": [
      { "data": "Operation", "sClass": "company_name" },
      { "data": "participant", "sClass": "integer" },
      { "data": "compagne", "sClass": "integer" },
  ...
  $(document).on('click', '#add_webmaster', function(e){
    e.preventDefault();
    $('.lightbox_content h2').text('Ajouter une opération Webmasters');
    ...
    $('#form_company #volume').val('');
    $('#form_company #nb').val('');
    $('#form_company #compagne').val('');
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/webmaster/table/php/data_webmaster.php?job=add_webmaster',
        cache:        false,
        data:         form_data,
        dataType:     'json',
      ...
      request.done(function(output){
        if (output.result == 'success'){
          table_companies.api().ajax.reload(function(){
            ...
            show_message("Opération webmaster '" + company_name + "' ajoutée avec succés.", 'success');
          }, true);
        } else {
          hide_loading_message();
  ...
  $(document).on('click', '#function_edit_web', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/webmaster/table/php/data_webmaster.php?job=get_webmaster_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company #compagne').val(output.data[0].compagne);
		$('#form_company #volume').val(output.data[0].volume);
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_company').serialize();
      var request   = $.ajax({
        url:          'module/webmaster/table/php/data_webmaster.php?job=edit_webmaster&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', '#del', function(e){
    ...
    if (confirm("Confirmation de supprission du '" + company_name + "' !!")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          'module/webmaster/table/php/data_webmaster.php?job=delete_webmaster&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
      ...
      request.done(function(output){
        if (output.result == 'success'){
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            show_message("Opération webmaster '" + company_name + "' effacer avec succès.", 'success');
          }, true);
        } else {
          hide_loading_message();
          show_message("Une erreur s'est produite lors de l'enregistrement", 'error');
