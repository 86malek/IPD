...
$(document).ready(function(){
  ...
  var table_cat_fichier = $('#table_gestion_doc_webs').dataTable({
    ...
    columnDefs: [{
  	...
  	"bStateSave": true,
    "ajax": "module/webmaster/table/php/data_webs_doc.php?job=get_doc",
    "columns": [
	  { "data": "id","sClass": "" },
	  { "data": "nom","sClass": "" },
	  { "data": "type","sClass": "" },
	  { "data": "taille","sClass": "" },
	  { "data": "actif","sClass": "" },
	  { "data": "ajout","sClass": "", "sType": "date-uk"},
	  { "data": "modif","sClass": "", "sType": "date-uk" },
	  { "data": "down","sClass": "" },
  ...
  $(document).on('click', '#function_edit', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/webmaster/table/php/data_webs_doc.php?job=get_doc_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
  ...
  $(document).on('submit', '#form_cat_fichier.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/webmaster/table/php/data_webs_doc.php?job=edit_doc&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
			...
			buttons: {
				confirm: {
					...
					action: function() {
						...
						var request = $.ajax({
						url:          'module/webmaster/table/php/data_webs_doc.php?job=delete_doc&id=' + id + '&cat=' + doc_up,
						cache:        false,
						dataType:     'json',
						contentType:  'application/json; charset=utf-8',
						type:         'get'
