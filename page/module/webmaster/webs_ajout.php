...
$query = $bdd->prepare("SELECT * FROM webmaster_doc WHERE doc_id = :id");
        ...
        <div class="container-fluid">
            ...
            <div class="col-lg-12">
              <div class="widget widget-welcome">
                ...
                <?php if(!empty($_GET['mode']) && $_GET['mode'] == 'update'){
                  	echo '<h4 class="widget-welcome__message-l1">Modification document WEBMASTERS (DOC)</h4>';
                }else{
                	echo '<h4 class="widget-welcome__message-l1">Chargement document WEBMASTERS (DOC)</h4>';
                }?>
			...
			{	
				...
				{ 
					...
					$extensions = array('.csv','.pdf','.ppt','.zip','.rar','.docx','.pptx');
					...
					if(!isset($erreur))
					{
						...
						if(move_uploaded_file($_FILES['files']['tmp_name'], $dossier . $fichier))
						{
								...
								$query = $bdd->prepare("INSERT INTO webmaster_doc (`doc_nom`, `doc`, `doc_date_ajout`, `doc_type`, `doc_taille`, `doc_actif`) VALUES (:doc_nom, :fichier, now(), :doc_type, :doc_taille, 1)");
								...
								$query->execute();
								$query->closeCursor();			
								echo "<script type='text/javascript'>document.location.replace('WebsDoc');</script>";
					...
					if(!isset($erreur))
					{
						...
						if(move_uploaded_file($_FILES['files']['tmp_name'], $dossier . $fichier))
						{
							...
							unlink($chemin);
								...
								$query = $bdd->prepare("UPDATE webmaster_doc SET `doc_nom` = :doc_nom,`doc` = :doc,`doc_date_modification` = now(),`doc_type` = :doc_type,`doc_taille` = :doc_taille WHERE doc_id = :id");
								$query->bindParam(":id", $id, PDO::PARAM_INT);
								$query->bindParam(":doc_nom", $_POST['doc_name'], PDO::PARAM_STR);
            ...
            <div id="formulaire">
            <?php if(!empty($_GET['mode']) && $_GET['mode'] == 'update'){?>
			<form method="post" action="WebsAjout-update-<?php echo $id; ?>.html"  enctype="multipart/form-data" onsubmit="afficheLoader('load');">
			<?php }else{ ?>
            <form method="post" action="WebsAjout"  enctype="multipart/form-data" onsubmit="afficheLoader('load');">
            <?php } ?>
                ...
                <div class="custom-alert__body">
                  ...
                  <div class="custom-alert__content">
                     <?php
								...
								$query = $bdd->prepare("SELECT doc FROM webmaster_doc WHERE doc_id = :id");
								$query->bindParam(":id", $id, PDO::PARAM_INT);
								$query->execute();
								$donnees = $query->fetch();								
								$query->closeCursor();
								echo '<b><a target="_blank" href="module/webmaster/upload/'.$donnees['doc'].'">'.$donnees['doc'].'</a></b>';
            ...
            <?php }?>  
            <div class="form-group">     
				<label for="read-only">Fichiers <b>CSV - PDF - PPT - ZIP - RAR - DOCX - PPTX</b> uniquement :</label><br><br>
            ...
            <a class="btn btn-danger  mr-3 icon-right" href="WebsDoc">Annuler et retour au tableau</a>	
            <?php }else{ ?>
