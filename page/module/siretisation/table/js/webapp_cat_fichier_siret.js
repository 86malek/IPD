...
$(document).ready(function(){
  ...
  var table_cat_fichier = $('#table_cat_fichier_acide').dataTable({
  	"bStateSave": true,
    "ajax": "module/siretisation/table/php/data_cat_fichier_siret.php?job=get_cat_fichier",
    "columns": [
	  { "data": "fichier","sClass": "" },
	  { "data": "statut","sClass": "" },
	  { "data": "total","sClass": "" },
    ...
    "oLanguage": {
      ...
      "sInfo":          "Total de _TOTAL_ Dossiers (Affichage _START_ à _END_)",
	  "sSearch":          "Recherche",
  ...
  $(document).on('click', '#function_edit_cat_fichier', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/siretisation/table/php/data_cat_fichier_siret.php?job=get_cat_fichier_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
  ...
  $(document).on('submit', '#form_cat_fichier.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/siretisation/table/php/data_cat_fichier_siret.php?job=edit_cat_fichier&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
			...
			buttons: {
				confirm: {
					...
					action: function() {
						...
						var request = $.ajax({
						url:          'module/siretisation/table/php/data_cat_fichier_siret.php?job=delete_cat_fichier&id=' + id + '&cat=' + doc_up,
						cache:        false,
						dataType:     'json',
						contentType:  'application/json; charset=utf-8',
						type:         'get'
