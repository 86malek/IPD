...
if ($job != ''){ 
  ...
  if ($job == 'get_cat_fichier_detail_rech'){
	...
	if($collab == ''){$requete_collab = ':user_id = :user_id';}else{$requete_collab = 'data_siret.user_id = :user_id';}
	
	$query_jour = $bdd->prepare("SELECT data_siret.id_siret, data_siret.id_cat_siretisation, data_siret.reporting, data_siret.user_name, data_siret.user_id, data_siret.date_calcul, data_siret.temps_sec, data_siret.etat FROM data_siret INNER JOIN data_cat_siretisation ON data_cat_siretisation.id_cat_siretisation = data_siret.id_cat_siretisation WHERE ".$requete_collab." AND data_siret.date_calcul between :debut and :fin AND data_siret.etat = 1 GROUP BY data_siret.date_calcul, data_siret.user_id, data_siret.id_cat_siretisation");
