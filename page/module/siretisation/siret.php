...
<div class="container-fluid m-tasks">
  <div class="main-container m-tasks__container container-heading-bordered">
    <h2 class="container-heading">
      <span>Bibliothèque Sirétisation (DATA)</span>
    ...
    <div class="container-body m-tasks__columns">
      ...
      <div class="m-tasks__column m-tasks__column--pending list-group">      
        ...
        <?php
			...
			$query = $bdd->prepare("SELECT count(*) FROM data_cat_siretisation WHERE id_cat_siretisation NOT IN (SELECT id_cat_siretisation FROM data_cat_synthese_siretisation)");
			$query->execute();
			$count = $query->fetchColumn();
			$query->closeCursor();
			if($count == 0){
			}else{
			$query = $bdd->prepare("SELECT * FROM data_cat_siretisation WHERE id_cat_siretisation NOT IN (SELECT id_cat_siretisation FROM data_cat_synthese_siretisation)");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
					echo '<a href="SiretBiblio-'.$donnees_attente['id_cat_siretisation'].'">       
					<div class="m-tasks__item list-group-item">            
					<h6 class="m-tasks__item-name"><b>'.$donnees_attente['nom_cat_siretisation'].'</b></h6>
      ...
      <div class="m-tasks__column m-tasks__column--in-progress list-group">
        ...
        <div class="m-tasks__items">
          <?php
			$query = $bdd->prepare("SELECT count(*) FROM data_cat_siretisation INNER JOIN data_cat_synthese_siretisation ON data_cat_synthese_siretisation.id_cat_siretisation = data_cat_siretisation.id_cat_siretisation WHERE data_cat_synthese_siretisation.niveau = 1 AND data_cat_synthese_siretisation.user_id = :user_id");
			...
			$query = $bdd->prepare("SELECT data_cat_siretisation.id_cat_siretisation, data_cat_siretisation.nom_cat_siretisation, data_cat_siretisation.fichier_cat_siretisation, data_cat_siretisation.date_ajout, data_cat_synthese_siretisation.user_id, data_cat_synthese_siretisation.user_name FROM data_cat_siretisation INNER JOIN data_cat_synthese_siretisation ON data_cat_synthese_siretisation.id_cat_siretisation = data_cat_siretisation.id_cat_siretisation WHERE data_cat_synthese_siretisation.niveau = 1 AND data_cat_synthese_siretisation.user_id = :user_id");
			$query->bindParam(":user_id", $_SESSION['user_id'], PDO::PARAM_INT);	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
				
				if($donnees_attente['user_id'] == $_SESSION['user_id']){					
				echo '<a href="SiretBiblio-debut-'.$donnees_attente['id_cat_siretisation'].'">       
				<div class="m-tasks__item list-group-item">            
				<h6 class="m-tasks__item-name">'.$donnees_attente['nom_cat_siretisation'].'</h6>
				<div class="m-tasks__item-desc">
      ...
      <div class="m-tasks__column m-tasks__column--in-progress list-group">
        ...
        <div class="m-tasks__items">
          ...
          <?php
			...
			$query = $bdd->prepare("SELECT data_cat_synthese_siretisation.niveau, data_cat_siretisation.statut_fichier_cat_siretisation, data_cat_siretisation.id_cat_siretisation, data_cat_siretisation.nom_cat_siretisation, data_cat_siretisation.	fichier_cat_siretisation, data_cat_siretisation.date_ajout, data_cat_synthese_siretisation.user_name FROM data_cat_siretisation INNER JOIN data_cat_synthese_siretisation ON data_cat_synthese_siretisation.id_cat_siretisation = data_cat_siretisation.id_cat_siretisation GROUP BY data_cat_synthese_siretisation.id_cat_siretisation");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
				
				$query_ligne = $bdd->prepare("SELECT * FROM data_siret WHERE reporting = 0 AND id_cat_siretisation = :id_cat_siretisation");
				$query_ligne->bindParam(":id_cat_siretisation", $donnees_attente['id_cat_siretisation'], PDO::PARAM_INT);
				$query_ligne->execute();
				$verif_ligne = $query_ligne->fetchColumn();
				$query_ligne->closeCursor();
				
				$query_etat = $bdd->prepare("SELECT * FROM data_cat_synthese_siretisation WHERE niveau = 1 AND id_cat_siretisation = :id_cat_siretisation");
				$query_etat->bindParam(":id_cat_siretisation", $donnees_attente['id_cat_siretisation'], PDO::PARAM_INT);
				$query_etat->execute();
      ...
      <div class="m-tasks__column m-tasks__column--completed list-group">
        ...
        <div class="m-tasks__items">
          ...
          <?php
			$query = $bdd->prepare("SELECT data_cat_siretisation.id_cat_siretisation, data_cat_siretisation.nom_cat_siretisation, data_cat_siretisation.fichier_cat_siretisation, data_cat_siretisation.date_ajout, data_cat_synthese_siretisation.user_name FROM data_cat_siretisation INNER JOIN data_cat_synthese_siretisation ON data_cat_synthese_siretisation.id_cat_siretisation = data_cat_siretisation.id_cat_siretisation GROUP BY data_cat_siretisation.id_cat_siretisation");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
