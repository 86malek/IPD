...
$query = $bdd->prepare("SELECT user_name, DATE_FORMAT(date_debut_traitment, '%d/%m/%Y à %H:%i:%S') AS DateTempsdebut, DATE_FORMAT(date_fin_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsfin, SEC_TO_TIME(temps_sec) AS traitement FROM cat_synthese_fiche_update WHERE linkedin_fiche_id = :linkedin_fiche_id ORDER BY linkedin_fiche_up_id DESC");	
$query->bindParam(":linkedin_fiche_id", $id_stat, PDO::PARAM_INT);
$query->execute();			
