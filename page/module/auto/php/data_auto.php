...
if ($job != ''){
  ...
  if ($job == 'get_auto'){
	...
	while ($rrows = mysqli_fetch_array($rs_qld_sql)) {
		...
		$update_qld = "UPDATE automobile SET ";
    ...
    $query_qld = "SELECT * FROM automobile";
    ...
    if ($id == ''){
      ...
      $query = "SELECT * FROM automobile WHERE id = '" . mysqli_real_escape_string($db_connection, $id) . "'";
    ...
    // Add company
    $query = "INSERT INTO automobile SET ";
    ...
    if ($id == ''){
      ...
      $query = "DELETE FROM automobile WHERE id = '" . mysqli_real_escape_string($db_connection, $id) . "'";
      $query = mysqli_query($db_connection, $query);
      if (!$query){
        $result  = 'error';
        $message = 'Échec de requête';
