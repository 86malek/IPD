...
if ($job != ''){
	...
	if ($job == 'get_rapport_dmp'){
	
	
	try
	{
	$PDO_query_rapport = $bdd->prepare("SELECT COUNT(dmp_champ_1) AS semaine, SUM(dmp_champ_2) AS email, dmp_liste_email, date_calcul, user_id, user_name, id_dmp FROM `dmp_traitment` GROUP BY `user_id`");
	...
	while ($traitement = $PDO_query_rapport->fetch()){	
		...
		$semaine = '<span class="badge badge-success ">'.$traitement['semaine'].' Semaine</span>';
		$email = '<span class="badge badge-warning ">'.$traitement['email'].' Email</span>';
		...
		$functions .= '<a href="DmpRapportCollab-'.$traitement['user_id'].'"><span class="badge badge-primary ">Affichage par semaines</span></a>';	
