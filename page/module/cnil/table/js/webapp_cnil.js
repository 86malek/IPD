...
$(document).ready(function(){
  ...
  var table_companies = $('#table_traitement_rapport').dataTable({
	"bStateSave": true,
    "ajax": "module/cnil/table/php/data_cnil.php?job=get_cnil&id_import=" + id_import,
    "columns": [
	  { "data": "date", "sClass": "" },
	  { "data": "ch1", "sClass": "" },
      { "data": "ch2", "sClass": "" },
  ...
  $(document).on('click', '#add_rapport', function(e){
    e.preventDefault();		
      $('.lightbox_content h2').text('NOUVELLE FICHE CNIL');
      $('#form_company button').text('ENREGISTREMENT');
      $('#form_company').attr('class', 'form add');
      $('#form_company').attr('data-id', '');
      $('#form_company .field_container label.error').hide();
      $('#form_company .field_container').removeClass('valid').removeClass('error');
      $('#form_company #ch1').val('');
      $('#form_company #ch2').val('');
	  $('#form_company #ch3').val('');
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/cnil/table/php/data_cnil.php?job=add_cnil',
        cache:        false,
        data:         form_data,
        dataType:     'json',
  ...
  $(document).on('click', '#function_edit_web', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/cnil/table/php/data_cnil.php?job=get_add_cnil',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('.lightbox_content h2').text('MODICIATION FICHE CNIL');
        $('#form_company button').text('ENREGISTREMENT');
		...
		$('#form_company #ch1').val(output.data[0].ch1);
		$('#form_company #ch2').val(output.data[0].ch2);
		$('#form_company #ch3').val(output.data[0].ch3);
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_company').serialize();
      var request   = $.ajax({
        url:          'module/cnil/table/php/data_cnil.php?job=edit_cnil&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', 'a#del', function(e){
		...
		t.confirm({
			title: cat_name,
			content: "Confirmation de supprission ",
			...
			buttons: {
				confirm: {
					...
					action: function() {
						...
						var request = $.ajax({
						url:          'module/cnil/table/php/data_cnil.php?job=delete_cnil&id=' + id,
						cache:        false,
						dataType:     'json',
						contentType:  'application/json; charset=utf-8',
						type:         'get'
