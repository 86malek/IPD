...
if(!empty($_POST['debut']) && $_POST['debut'] == 'debut'){
	...
	$query = $bdd->prepare("SELECT count(*) FROM data_cat_synthese_ie WHERE id_cat_ie = :id AND niveau = 1 AND user_id = :user_id");
	...
	if ($rowcount == 0){
			...
			$query = $bdd->prepare("INSERT INTO data_cat_synthese_ie SET user_name = :user_name, user_id = :user_id, statut_cat_fichier_ie = 2, id_cat_ie = :id, niveau = 1");
			$query->bindParam(":id", $id, PDO::PARAM_INT);
			$query->bindParam(":user_id", $_SESSION['user_id'], PDO::PARAM_INT);
			...
			$query = $bdd->prepare("UPDATE data_cat_synthese_ie SET statut_cat_fichier_ie = 1, niveau = 2 WHERE id_cat_ie = :id_cat_ie AND user_id = :user_id");
...
$doc = $query->fetch();
$query->closeCursor();	
$document = $doc['nom_cat_ie'];

$datetime = date("Y-m-d");
$query_ligne_taiter = $bdd->prepare("SELECT count(*) FROM `data_ie` WHERE reporting <> 0 AND user_id = :user_id AND date_calcul = :date_calcul");
...
$query = $bdd->prepare("SELECT nbligne_objectif, nbheure_objectif FROM data_cat_synthese_fiche_obj_ie WHERE debut_objectf <= '".$datetime."' AND fin_objectif >= '".$datetime."' ORDER BY id_objectif DESC LIMIT 0, 1");
$query->execute();
$donnees = $query->fetch();
...
<div class="page-content">
        ...
        <div class="container-fluid">
          ...
          <div class="main-container">
            ...
            <a class="btn btn-success btn-sm icon-left mr-3" href="IE">Retour aux statistique <span class="btn-icon iconfont iconfont-step-arrow-left"></span></a>
            
            <?php }else{ ?> 
				<?php if(!empty($_GET['mode']) && $_GET['mode'] == 'debut'){?>                
                <form id="fin" method="post" action="IEBiblio-fin-<?php echo $_GET['id']; ?>">
                ...
                <a class="btn btn-info icon-left btn-sm mr-3" href="IEBiblio">Retour à la liste<span class="btn-icon iconfont iconfont-info"></span></a>
                </form>    
                
                <?php }else{?>
                
                <form id="debut" method="post" action="IEBiblio-debut-<?php echo $_GET['id']; ?>">
			...
			$query = $bdd->prepare("SELECT * FROM `data_ie` WHERE (id_cat_ie = :idcat AND user_id = :user_id) OR (id_cat_ie = :idcat AND user_id = 0) GROUP BY reporting");
			...
			if($query_qualif['reporting'] == 0){
				echo' <a class="btn btn-primary icon-right btn-sm mr-3" href="IEBiblio-debut-'.$_GET['id'].'">Supprimer le filtre</a>';
				echo' <a class="btn btn-warning icon-right btn-sm mr-3" href="IEBiblioQalif-debut-'.$_GET['id'].'-'.$query_qualif['reporting'].'">EN ATTENTE</a>';	
						

			}elseif($query_qualif['reporting'] == 1){
				echo' <a class="btn btn-success icon-right  btn-sm mr-3" href="IEBiblioQalif-debut-'.$_GET['id'].'-'.$query_qualif['reporting'].'">OK</a>';
				
				
			}elseif($query_qualif['reporting'] == 2){
				echo' <a class="btn btn-danger icon-right btn-sm mr-3" href="IEBiblioQalif-debut-'.$_GET['id'].'-'.$query_qualif['reporting'].'">KO</a>';
...
<div class="lightbox_container">
  ...
  <div class="lightbox_content">
            ...
            <hr>
            <div class="input_container">
                <label for="nature">ETB :</label>
                        <div class="field_container">
                        <input type="text" maxlength="15" class="form-control" id="etb" name="etb" disabled>
                    </div>
            </div>
            
            <div class="input_container">
                <label for="nature">CA BRE :</label>
                    <div class="field_container">
                        <input type="text" maxlength="15" class="form-control" id="bre" name="bre" disabled>
                    </div>
            </div> 
            
            <div class="input_container">
                <label for="nature">ANNÉE CA BRE :</label>
                    <div class="field_container">
                        <input type="text" maxlength="15" class="form-control" id="annee" name="annee" disabled>
                    </div>
            </div> 
            	...
            	<label for="nature">STATUT : <span class="required">*</span></label>
                    <div class="field_container">
                        	<select id="reporting" name="reporting" class="form-control" required>
                                ...
                                <option value="5">RAPPEL</option>
                                <option value="6">HS / FAX</option>
                                <option value="7">FAUX NUMÉRO</option>
                                <option value="8">CESSATION D'ACTIVITÉ</option>
            ...
            <hr>
            <p style="color:#fe6f60">Partie : <b>Administration</b></p>
            <hr>
            
             
            <div class="form-group">
              <textarea rows="4" placeholder="..." class="form-control" id="commentaire" name="commentaire" readonly></textarea>
            </div>
            <div class="form-group" style="text-align:right">
...
<?php if (checkAdmin()) {?>
<script charset="utf-8" src="module/ie/table/js/webapp_ie_traitement_admin.js"></script>
<?php }else{ ?>
<script charset="utf-8" src="module/ie/table/js/webapp_ie_traitement.js"></script>
<?php }?>
