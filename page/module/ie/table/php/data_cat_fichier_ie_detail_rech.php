...
if ($job != ''){ 
  ...
  if ($job == 'get_cat_fichier_detail_rech'){
	...
	if($collab == ''){$requete_collab = ':user_id = :user_id';}else{$requete_collab = 'data_ie.user_id = :user_id';}
	
	$query_jour = $bdd->prepare("SELECT data_ie.id_ie, data_ie.id_cat_ie, data_ie.reporting, data_ie.user_name, data_ie.user_id, data_ie.date_calcul, data_ie.temps_sec, data_ie.etat FROM data_ie INNER JOIN data_cat_ie ON data_cat_ie.id_cat_ie = data_ie.id_cat_ie WHERE ".$requete_collab." AND data_ie.date_calcul between :debut and :fin AND data_ie.etat = 1 GROUP BY data_ie.date_calcul, data_ie.user_id, data_ie.id_cat_ie");
