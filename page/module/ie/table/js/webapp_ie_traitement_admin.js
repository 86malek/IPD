...
$(document).ready(function(){
  ...
  var table_companies = $('#table_traitement').dataTable({
    ...
    "ajax": "module/ie/table/php/data_ie_traitement.php?job=get_traitement_admin&id_import=" + id_import,
    "columns": [
		{ "data": "alerte",      "sClass": "" },
		{ "data": "etb", "sClass": "company_name" },
		{ "data": "bre", "sClass": "company_name" },
		{ "data": "annee", "sClass": "company_name" },
  ...
  $(document).on('click', '#function_edit_web', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/ie/table/php/data_ie_traitement.php?job=get_traitement_add_admin',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
    ...
    request.done(function(output){
      if (output.result == 'success'){
		...
		$('#form_company #etb').val(output.data[0].etb);
		$('#form_company #bre').val(output.data[0].bre);
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/ie/table/php/data_ie_traitement.php?job=edit_traitement_admin&id=' + id,
        cache:        true,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', '#mood_affichage', function(){
      var temps_reel      = $(this).data('id');
		t.dialog({
			...
			content: "url:module/ie/table/data/mood-table.php?id_stat=" + temps_reel,
			animation: 'zoom',
			columnClass: 'medium',
