...
if ($job != ''){  
  ...
  if ($job == 'get_cat_fichier'){
    ...
    /*try 
	{*/
	$query_global = $bdd->prepare("SELECT * FROM `hb_cat_acide` ORDER BY `id_cat_acide` DESC");
	...
	while ($doc = $query_global->fetch()){
		...
		$query = $bdd->prepare("SELECT MAX(date_fin_traitement) AS date_fin, MIN(date_debut_traitment) AS date_debut FROM hb_cat_synthese_fiche_update WHERE id_cat_acide = :id_cat_acide");
		$query->bindParam(":id_cat_acide", $doc['id_cat_acide'], PDO::PARAM_INT);
		$query->execute();
		...
		$query_somme_total_ligne = $bdd->prepare("SELECT count(*) FROM `hb_acide` WHERE id_cat_acide = :id_cat_acide");
		...
		$query_somme_traite_ligne = $bdd->prepare("SELECT count(*) FROM `hb_acide` WHERE id_cat_acide = :id_cat_acide AND reporting <> 0");
		...
		$query_somme_participant = $bdd->prepare("SELECT COUNT(DISTINCT(user_id)) FROM `hb_acide` WHERE id_cat_acide = :id_cat_acide AND user_id <> 0");
		...
		$query_save = $bdd->prepare("UPDATE hb_cat_acide SET intervenant_cat_acide = :intervenant_cat_acide WHERE id_cat_acide = :id_cat_acide");
		...
		$query = $bdd->prepare("SELECT SEC_TO_TIME(SUM(temps_sec)) AS datee FROM hb_cat_synthese_fiche_update WHERE id_cat_acide = :id_cat_acide");
		$query->bindParam(":id_cat_acide", $doc['id_cat_acide'], PDO::PARAM_INT);
		$query->execute();	
		...
		$somme_traite_pourcentage_couleur = round(($somme_traite/$somme_total)*100);
			...
			$somme_traite_pourcentage = '<span class="badge badge-danger badge-rounded mb-3 mr-3">'.round(($somme_traite/$somme_total)*100).'%</span>';
			...
			}elseif($somme_traite_pourcentage_couleur == 0){
		...
		$query_calcul_update = $bdd->prepare("SELECT count(*) FROM hb_cat_synthese_acide WHERE id_cat_acide IN (SELECT id_cat_acide FROM hb_cat_synthese_acide WHERE id_cat_acide = :id_cat_acide)");
		...
		if ($rowcountupdate > 0){
			...
			$query_calcul_statut = $bdd->prepare("SELECT count(*) FROM hb_cat_synthese_acide WHERE id_cat_acide = :id_cat_acide  AND niveau <> 2");
			...
			if ($rowcountst > 0){
				$query_update_statut = $bdd->prepare("UPDATE hb_cat_acide SET statut_cat_fichier = 2 WHERE id_cat_acide = :id_cat_acide");
				...
				$query_verif_ligne = $bdd->prepare("SELECT count(*) FROM hb_acide WHERE reporting = 0 AND id_cat_acide = :id_cat_acide");
				...
				if($verif_ligne > 0){
					...
					$query_update_statut = $bdd->prepare("UPDATE hb_cat_acide SET statut_cat_fichier = 1 WHERE id_cat_acide = :id_cat_acide");
					$query_update_statut->bindParam(":id_cat_acide", $doc['id_cat_acide'], PDO::PARAM_INT);
					$query_update_statut->execute();
					$query_update_statut->closeCursor();
				}
			...
			$query_update_statut = $bdd->prepare("UPDATE hb_cat_acide SET statut_cat_fichier = 3 WHERE id_cat_acide = :id_cat_acide");
		...
		$functions .= '<a href="HBAjout-update-' . $doc['id_cat_acide'] . '.html"><span class="badge badge-shamrock mb-3 mr-3">Modifier fichier</span></a>';
		...
		$functions .= '<a target="_blank" href="module/hb/upload/' . $doc['fichier_cat_acide'] . '"><span class="badge badge-success mb-3 mr-3"><span class="icon iconfont iconfont-file-excel"></span></span></a>';
		$functions .= '<a href="HBBiblio-' . $doc['id_cat_acide'] . '"><span class="badge badge-primary mb-3 mr-3">Données</span></a>';
		$functions .= '<a href="#" id="function_edit_cat_fichier" data-id="'   . $doc['id_cat_acide'] . '" data-name="Lot : ' . $doc['nom_cat_acide'] . '"><span class="badge badge-lasur"><span class="icon iconfont iconfont-pencil"></span></span></a>';	
			...
			$functions .= '<a href="HBBiblioDetails-' . $doc['id_cat_acide'] . '"><span class="badge badge-primary mb-3 mr-3">Cumul</span></a>';
			$functions .= '<a href="HBBiblioDetailsJour-' . $doc['id_cat_acide'] . '"><span class="badge badge-primary mb-3 mr-3">Journalier</span></a>';
			$functions .= '<a href="HBBiblio-' . $doc['id_cat_acide'] . '"><span class="badge badge-primary mb-3 mr-3">Données</span></a>';
			$functions .= '<a target="_blank" href="module/hb/upload/' . $doc['fichier_cat_acide'] . '"><span class="badge badge-success mb-3 mr-3"><span class="icon iconfont iconfont-file-excel"></span></span></a>';
			$functions .= '<a href="#" id="function_edit_cat_fichier" data-id="'   . $doc['id_cat_acide'] . '" data-name="Lot : ' . $doc['nom_cat_acide'] . '"><span class="badge badge-lasur"><span class="icon iconfont iconfont-pencil"></span></span></a>';
		}		
    ...
    if ($id == ''){
      ...
      $message = 'Échec id';
		...
		$query = $bdd->prepare("UPDATE hb_cat_acide SET nom_cat_acide = :nom_cat_acide WHERE id_cat_acide = :id_cat_acide");
		$query->bindParam(":id_cat_acide", $id, PDO::PARAM_INT);
	...
	try
	{
	$PDO_query_traitement = $bdd->prepare("SELECT * FROM `hb_cat_synthese_fiche_obj` ORDER BY id_objectif ASC");
    ...
    if ($id == ''){
      ...
      $message = 'Échec id';
		...
		try
		{
		$query = $bdd->prepare("SELECT * FROM hb_cat_synthese_fiche_obj WHERE id_objectif = :id");
		...
		$query = $bdd->prepare("INSERT INTO hb_cat_synthese_fiche_obj (nbligne_objectif, nbheure_objectif, debut_objectf, fin_objectif) VALUES (:nbligne_objectif, :nbheure_objectif, :debut_objectf, :fin_objectif)");
    ...
    if ($id == ''){
      ...
      $message = 'Échec id';
   		...
   		try
		{
			$debut = substr($date, 0,10);
		$fin = substr($date, 13,22);
		$query = $bdd->prepare("UPDATE hb_cat_synthese_fiche_obj SET nbligne_objectif = :nbligne_objectif, nbheure_objectif = :nbheure_objectif, debut_objectf = :debut_objectf, fin_objectif = :fin_objectif WHERE id_objectif = :id");
		$query->bindParam(":id", $id, PDO::PARAM_INT);
      ...
      $message = 'Échec id';
    	...
    	try
		{
		$query = $bdd->prepare("DELETE FROM hb_cat_synthese_fiche_obj WHERE id_objectif = :id");
		$query->bindParam(":id", $id, PDO::PARAM_INT);
		$query->execute();
		$query->closeCursor();
		$result  = 'success';
	...
	if ($id == ''){
      ...
      $message = 'Échec id';
		...
		$result  = 'success';
		$message = 'Succès de requête';
		$query_del_niveau1 = $bdd->prepare("DELETE FROM hb_cat_acide WHERE id_cat_acide = :id");	
