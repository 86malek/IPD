...
$(document).ready(function(){
  ...
  var table_cat_fichier = $('#table_cat_fichier_acide').dataTable({
  	"bStateSave": true,
    "ajax": "module/hb/table/php/data_cat_fichier_acide.php?job=get_cat_fichier",
    "columns": [
	  { "data": "fichier","sClass": "" },
	  { "data": "statut","sClass": "" },
	  { "data": "total","sClass": "" },
  ...
  $(document).on('click', '#function_edit_cat_fichier', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'module/hb/table/php/data_cat_fichier_acide.php?job=get_cat_fichier_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
    ...
    request.done(function(output){
      if (output.result == 'success'){
        $('.lightbox_content h2').text('Modification du nom du dossier');
        $('#form_cat_fichier button').text('Enregistrement de la modification');
  ...
  $(document).on('submit', '#form_cat_fichier.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'module/hb/table/php/data_cat_fichier_acide.php?job=edit_cat_fichier&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
			  ...
			  sayMyName: {
				...
				action: function () {
				  ...
				  if (input_ligne.val() == '' || input_heure.val() == '') {
					...
					var request = $.ajax({
					url:          'module/linkedin/table/data/objectif-table.php?nbheure=' + input_heure.val() + '&nblignes=' + input_ligne.val() + '&section=' + input_section.val(),
					cache:        false,
					dataType:     'json',
					contentType:  'application/json; charset=utf-8',
					type:         'get'
			...
			buttons: {
				confirm: {
					...
					action: function() {
						...
						var request = $.ajax({
						url:          'module/hb/table/php/data_cat_fichier_acide.php?job=delete_cat_fichier&id=' + id + '&cat=' + doc_up,
						cache:        false,
						dataType:     'json',
						contentType:  'application/json; charset=utf-8',
						type:         'get'
