...
$query = $bdd->prepare("SELECT user_name, DATE_FORMAT(date_debut_traitment, '%d/%m/%Y à %H:%i:%S') AS DateTempsdebut, DATE_FORMAT(date_fin_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsfin, SEC_TO_TIME(temps_sec) AS traitement FROM hb_cat_synthese_fiche_update WHERE id_acide = :id_acide ORDER BY id_fiche_up DESC");	
$query->bindParam(":id_acide", $id_stat, PDO::PARAM_INT);
$query->execute();			
