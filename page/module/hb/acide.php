...
<div class="container-fluid m-tasks">
  <div class="main-container m-tasks__container container-heading-bordered">
    <h2 class="container-heading">
      <span>Bibliothèque HARD BOUNCE (HB)</span>
    ...
    <div class="container-body m-tasks__columns">
      ...
      <div class="m-tasks__column m-tasks__column--pending list-group">      
        ...
        <?php
			...
			$query = $bdd->prepare("SELECT count(*) FROM hb_cat_acide WHERE id_cat_acide NOT IN (SELECT id_cat_acide FROM hb_cat_synthese_acide)");
			$query->execute();
			$count = $query->fetchColumn();
			$query->closeCursor();
			if($count == 0){
			}else{
			$query = $bdd->prepare("SELECT * FROM hb_cat_acide WHERE id_cat_acide NOT IN (SELECT id_cat_acide FROM hb_cat_synthese_acide)");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
					echo '<a href="HBBiblio-'.$donnees_attente['id_cat_acide'].'">       
      ...
      <div class="m-tasks__column m-tasks__column--in-progress list-group">
        ...
        <div class="m-tasks__items">
          <?php
			$query = $bdd->prepare("SELECT count(*) FROM hb_cat_acide INNER JOIN hb_cat_synthese_acide ON hb_cat_synthese_acide.id_cat_acide = hb_cat_acide.id_cat_acide WHERE hb_cat_synthese_acide.niveau = 1");
			$query->execute();
			$count = $query->fetchColumn();
			$query->closeCursor();
			if($count == 0){		
			...
			$query = $bdd->prepare("SELECT hb_cat_acide.id_cat_acide, hb_cat_acide.nom_cat_acide, hb_cat_acide.fichier_cat_acide, hb_cat_acide.date_ajout_cat_acide, hb_cat_synthese_acide.id_intervenant_cat_acide, hb_cat_synthese_acide.intervenant_cat_acide FROM hb_cat_acide INNER JOIN hb_cat_synthese_acide ON hb_cat_synthese_acide.id_cat_acide = hb_cat_acide.id_cat_acide WHERE hb_cat_synthese_acide.niveau = 1");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
				
				if($donnees_attente['id_intervenant_cat_acide'] == $_SESSION['user_id']){					
				echo '<a href="HBBiblio-debut-'.$donnees_attente['id_cat_acide'].'">       
				<div class="m-tasks__item list-group-item">            
				<h6 class="m-tasks__item-name">'.$donnees_attente['nom_cat_acide'].'</h6>
				<div class="m-tasks__item-desc">
      ...
      <div class="m-tasks__column m-tasks__column--completed list-group">
        ...
        <div class="m-tasks__items">
          ...
          <?php
			...
			$query = $bdd->prepare("SELECT hb_cat_synthese_acide.niveau, hb_cat_acide.statut_cat_fichier, hb_cat_acide.id_cat_acide, hb_cat_acide.nom_cat_acide, hb_cat_acide.fichier_cat_acide, hb_cat_acide.date_ajout_cat_acide, hb_cat_synthese_acide.intervenant_cat_acide FROM hb_cat_acide INNER JOIN hb_cat_synthese_acide ON hb_cat_synthese_acide.id_cat_acide = hb_cat_acide.id_cat_acide GROUP BY hb_cat_synthese_acide.id_cat_acide");	
			$query->execute();			
			while ($donnees_attente = $query->fetch()){
				
				$query_ligne = $bdd->prepare("SELECT * FROM hb_acide WHERE reporting = 0 AND id_cat_acide = :id_cat_acide");
				...
				$query_etat = $bdd->prepare("SELECT * FROM hb_cat_synthese_acide WHERE niveau = 1 AND id_cat_acide = :id_cat_acide");
