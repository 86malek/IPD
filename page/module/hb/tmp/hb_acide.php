...
<div class="container-fluid m-tasks">
  <div class="main-container m-tasks__container container-heading-bordered">
    <h2 class="container-heading">
      <span>Bibliothèque des fichiers Hard Bounce (HB)</span>
    </h2>
    <div class="container-body m-tasks__columns">
      ...
      <div class="m-tasks__column m-tasks__column--pending list-group">      
        ...
        <div class="m-tasks__column-header">
          ...
          <span class="m-tasks__column-name">Cartes EN ATTENTES</span>
        </div>
        <div class="m-tasks__items"> 
        <?php
				
			$attente = mysqli_query($db,"SELECT * FROM hb_cat_acide WHERE id_cat_acide NOT IN (SELECT id_cat_acide FROM hb_cat_synthese_acide)") or die(mysqli_connect_error());
        ...
        <div class="m-tasks__items">
          <?php
			...
			while($donnees_attente = mysqli_fetch_array($attente)){
				...
				if($donnees_attente['intervenant_cat_acide'] == $_SESSION['user_name']){
      ...
      <div class="m-tasks__column m-tasks__column--completed list-group">
        <div class="m-tasks__column-header">
          <span class="m-tasks__column-icon iconfont iconfont-task-label"></span>
          <span class="m-tasks__column-name">Cartes EN PROGRESSION</span>
        </div>
        <div class="m-tasks__items">
          
          <?php
			
			$attente = mysqli_query($db,"SELECT hb_cat_synthese_acide.niveau, hb_cat_acide.statut_cat_fichier, hb_cat_acide.id_cat_acide, hb_cat_acide.nom_cat_acide, hb_cat_acide.fichier_cat_acide, hb_cat_acide.date_ajout_cat_acide, hb_cat_synthese_acide.intervenant_cat_acide FROM hb_cat_acide INNER JOIN hb_cat_synthese_acide ON hb_cat_synthese_acide.id_cat_acide = hb_cat_acide.id_cat_acide GROUP BY hb_cat_synthese_acide.id_cat_acide") or die(mysqli_connect_error());
			while($donnees_attente = mysqli_fetch_array($attente)){	
			
			$verif_ligne = mysqli_query($db,"SELECT * FROM acide WHERE reporting IS NULL AND id_cat_acide =".$donnees_attente['id_cat_acide']."") or die(mysqli_connect_error());
			$verif_ligne = mysqli_num_rows($verif_ligne);
			
			$verif_etat = mysqli_query($db,"SELECT * FROM hb_cat_synthese_acide WHERE niveau = 1 AND id_cat_acide =".$donnees_attente['id_cat_acide']."") or die(mysqli_connect_error());
      ...
      <div class="m-tasks__column m-tasks__column--completed list-group">
        <div class="m-tasks__column-header">
          <span class="m-tasks__column-icon iconfont iconfont-task-label"></span>
          <span class="m-tasks__column-name">Cartes TERMINÉES</span>
        </div>
        <div class="m-tasks__items">
          
          <?php
			
			$attente = mysqli_query($db,"SELECT hb_cat_acide.id_cat_acide, hb_cat_acide.nom_cat_acide, hb_cat_acide.fichier_cat_acide, hb_cat_acide.date_ajout_cat_acide, hb_cat_synthese_acide.intervenant_cat_acide FROM hb_cat_acide INNER JOIN hb_cat_synthese_acide ON hb_cat_synthese_acide.id_cat_acide = hb_cat_acide.id_cat_acide GROUP BY hb_cat_acide.id_cat_acide") or die(mysqli_connect_error());
			while($donnees_attente = mysqli_fetch_array($attente)){	
