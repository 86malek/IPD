...
$query = $bdd->prepare("SELECT nbligne_objectif, nbheure_objectif FROM hb_cat_synthese_fiche_obj WHERE debut_objectf <= '".date("Y-m-d")."' AND fin_objectif >= '".date("Y-m-d")."' ORDER BY id_objectif DESC LIMIT 0, 1");
$query->execute();
$donnees = $query->fetch();
...
$query->closeCursor();
          ...
          $query = $bdd->prepare("SELECT count(*) FROM `hb_acide` WHERE reporting = 0");
          $query->execute();
          $total_nt = $query->fetchColumn();
          $query->closeCursor();
          $total_ntt = ''.$total_nt.'';

          $query = $bdd->prepare("SELECT count(*) FROM `hb_acide` WHERE reporting <> 0");
          $query->execute();
          $total_t = $query->fetchColumn();
          $query->closeCursor();
          $total_tt = ''.$total_t.'';

          $somme_traite_pourcentage = round(($total_t/$total_nt)*100);

          if($somme_traite_pourcentage > 50 && $somme_traite_pourcentage < 100){
          $somme_traite_pourcentage_c = '<span class="">'.round(($total_t/$total_nt)*100).'%</span>';
          }elseif($somme_traite_pourcentage < 50 && $somme_traite_pourcentage > 0){
          $somme_traite_pourcentage_c = '<span class="">'.round(($total_t/$total_nt)*100).'%</span>';
          }elseif($somme_traite_pourcentage == 100){
          $somme_traite_pourcentage_c = '<span class="">'.round(($total_t/$total_nt)*100).'%</span>';
          }elseif($somme_traite_pourcentage == 0){
          $somme_traite_pourcentage_c = '<span class="">'.round(($total_t/$total_nt)*100).'%</span>';
          }
...
?>
	...
	<div class="page-content">
        ...
        <div class="container-fluid">     
          <h2 class="content-heading">DOSSIERS HARD BOUNCE (HB)</h2>
          ...
          <div class="main-container">
            ...
            <div class="row">
            <div class="col-lg-6">
            <a class="btn btn-success icon-left btn-sm mr-3" href="HBAjout">Ajouter un fichier pour traitement<span class="btn-icon iconfont iconfont-plus-v1"></span></a>
            <a class="btn btn-secondary icon-left btn-sm mr-3" href="HBObj">Gestion des objectifs</span></a>
        	...
        	<div id="modal-settings" class="modal fade custom-modal-tabs">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <form action="HBRech" method="post" name="filtre">
                          ...
                          <div class="col-lg-12">
                            <div class="form-group">
                              ...
                              <select class="form-control" data-placeholder="Tous les intervenants disponibles" name="collab">
                                <option></option>
                                <?php
                								$query = $bdd->prepare("SELECT operateur_acide, user_id FROM `hb_acide` WHERE user_id <> 0 GROUP BY user_id");
                								$query->execute();	
            ...
            <th></th>
            <th></th>
            <th><?php echo $total_ntt; ?></th>
            <th><?php echo $total_tt; ?></th>
            <th></th>
            <th></th>
            <th></th>
            <th></th>
  ...
  <div class="lightbox_content">
          ...
          <hr> 
            ...
            <div class="input_container">
            	<label for="reporting">NOM DU DOSSIER : <span class="required">*</span></label>
...
<script src="module/hb/table/js/webapp_cat_fichier_acide.js"></script>
