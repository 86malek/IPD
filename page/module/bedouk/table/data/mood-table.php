...
$query = $bdd->prepare("SELECT user_name, DATE_FORMAT(date_debut_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsdebut, DATE_FORMAT(date_fin_traitement, '%d/%m/%Y à %H:%i:%S') AS DateTempsfin, SEC_TO_TIME(temps_sec) AS traitement FROM data_cat_synthese_fiche_update_siretisation WHERE id_siret = :id_siret ORDER BY fiche_update_id DESC");	
$query->bindParam(":id_siret", $id_stat, PDO::PARAM_INT);
$query->execute();			
