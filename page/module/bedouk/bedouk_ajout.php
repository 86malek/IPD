...
$query = $bdd->prepare("SELECT * FROM data_cat_siretisation WHERE id_cat_siretisation = :id");
...
?>
	...
	<div class="page-content">
        <div class="container-fluid">
            ...
            <div class="col-lg-6">
              <div class="widget widget-welcome">
                ...
                <?php if(!empty($_GET['mode']) && $_GET['mode'] == 'update'){
                  	echo '<h4 class="widget-welcome__message-l1">Modification dossier Sirétisation (DATA)</h4>';
                }else{
                	echo '<h4 class="widget-welcome__message-l1">Chargement dossier Sirétisation (DATA)</h4>';
                }?>
				...
				{ 
					...
					if(!isset($erreur))
					{
						...
						if(move_uploaded_file($_FILES['files']['tmp_name'], $dossier . $fichier))
						{
								...
								$query = $bdd->prepare("INSERT INTO data_cat_siretisation (`nom_cat_siretisation`, `fichier_cat_siretisation`, `date_ajout`) VALUES (:nom_cat_siretisation, :fichier, now())");
								$query->bindParam(":nom_cat_siretisation", $_POST['doc_name'], PDO::PARAM_STR);
								$query->bindParam(":fichier", $fichier, PDO::PARAM_STR);
								$query->execute();
								$query->closeCursor();
								
								$query = $bdd->prepare("SELECT MAX(id_cat_siretisation)as max FROM data_cat_siretisation");
								$query->execute();
								...
								$query = $bdd->prepare("INSERT INTO data_siret (id_cat_siretisation, url_siretisation) VALUES (:id_cat_siretisation, :url_siretisation)");
								
								$query->bindParam(":id_cat_siretisation", $donnees_max['max'], PDO::PARAM_INT);
								$query->bindParam(":url_siretisation", utf8_encode($col[0]), PDO::PARAM_STR);
								$query->execute();
								$query->closeCursor();
								...
								echo "<script type='text/javascript'>document.location.replace('Siret');</script>";
				...
				{ 
					...
					if(!isset($erreur))
					{
						...
						if(move_uploaded_file($_FILES['files']['tmp_name'], $dossier . $fichier))
						{
							...
							if(file_exists($dossier . $donnees_update['fichier_cat_siretisation']))
							{
							$chemin = "upload/".$donnees_update['fichier_cat_siretisation'];						
							...
							unlink($chemin);
								...
								$query = $bdd->prepare("UPDATE data_cat_siretisation SET `nom_cat_siretisation` = :nom_cat_siretisation,`fichier_cat_siretisation` = :fichier_cat_siretisation,`date_modification` = now() WHERE id_cat_siretisation = :id");
								$query->bindParam(":id", $id, PDO::PARAM_INT);
								$query->bindParam(":nom_cat_siretisation", $_POST['doc_name'], PDO::PARAM_STR);
								$query->bindParam(":fichier_cat_siretisation", $fichier, PDO::PARAM_STR);
								$query->execute();
								$query->closeCursor();							
            ...
            <div id="formulaire">
            <?php if(!empty($_GET['mode']) && $_GET['mode'] == 'update'){?>
			<form method="post" action="SiretAjout-update-<?php echo $id; ?>.html"  enctype="multipart/form-data" onsubmit="afficheLoader('load');">
			<?php }else{ ?>
            <form method="post" action="SiretAjout"  enctype="multipart/form-data" onsubmit="afficheLoader('load');">
            <?php } ?>
			<div class="form-group">
          	<label for="read-only">Nom du (fichier / Dossier) :</label>
			<input type="text" placeholder="Aucune restriction de taille." class="form-control" name="doc_name" id="doc_name" value="<?php if(!empty($_GET['mode']) && $_GET['mode'] == 'update'){ echo $donnees_update['nom_cat_siretisation'];}?>" required>
			</div>
                ...
                <div class="custom-alert__body">
                  ...
                  <div class="custom-alert__content">
                     <?php
								...
								$query = $bdd->prepare("SELECT fichier_cat_siretisation FROM data_cat_siretisation WHERE id_cat_siretisation = :id");
								$query->bindParam(":id", $id, PDO::PARAM_INT);
								$query->execute();
								$donnees = $query->fetch();								
								$query->closeCursor();
								echo '<b><a target="_blank" href="module/siretisation/upload/'.$donnees['fichier_cat_siretisation'].'">'.$donnees['fichier_cat_siretisation'].'</a></b>';
            ...
            <a class="btn btn-danger  mr-3 icon-right" href="Siret">Annuler et retour à la bibliothèque</a>	
            <?php }else{ ?>
