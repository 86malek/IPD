...
if ($job != ''){
  ...
  if ($job == 'get_qld'){
	...
	$rs_qld = "SELECT * FROM qualite_de_donnees";
	...
	while ($rrows = mysqli_fetch_array($rs_qld_sql)) {
		...
		$update_qld = "UPDATE qualite_de_donnees SET ";
    ...
    $query_qld = "SELECT * FROM qualite_de_donnees ORDER BY id DESC";
    ...
    if (!$query_qld){
      ...
      while ($qld = mysqli_fetch_array($query_qld)){
        ...
        $functions  = '<center>';
        $functions .= '<a href="#" id="function_edit_web" data-id="'   . $qld['id'] . '" data-name="' . $qld['operation_qld'] . '"><span class="badge badge-success badge-rounded mb-3 mr-3">Modifier</span></a>';
        $functions .= '<a href="#" id="del" data-id="' . $qld['id'] . '" data-name="' . $qld['operation_qld'] . '"><span  class="badge badge-danger badge-rounded mb-3 mr-3">Effacer</span></a>';
        $functions .= '</center>';
  ...
  } elseif ($job == 'get_qld_add'){
    
    if ($id == ''){
      $result  = 'error';
      $message = 'Échec id';
    } else {
      $query = "SELECT * FROM qualite_de_donnees WHERE id = '" . mysqli_real_escape_string($db_connection, $id) . "'";
    ...
    $query = "INSERT INTO qualite_de_donnees SET ";
    ...
    if ($id == ''){
      ...
      $query = "UPDATE qualite_de_donnees SET ";
		if (isset($_GET['operation']))         { $query .= "operation_qld         = '" . mysqli_real_escape_string($db_connection, $_GET['operation'])         . "', "; }
    if (isset($_GET['nb'])) { $query .= "nb_participant_qld = '" . mysqli_real_escape_string($db_connection, $_GET['nb']) . "', "; }
    if (isset($_GET['object']))   { $query .= "objectif_qld   = '" . mysqli_real_escape_string($db_connection, $_GET['object'])   . "', "; }
	if (isset($_GET['nature']))   { $query .= "nature_objet_qld   = '" . mysqli_real_escape_string($db_connection, $_GET['nature'])   . "', "; }
