...
if ($job != ''){  
  ...
  if ($job == 'get_doc'){
    ...
    /*try 
	{*/
	$query_global = $bdd->prepare("SELECT * FROM `energis_doc` ORDER BY `doc_id` DESC");
	...
	while ($doc = $query_global->fetch()){
		...
		$functions .= '<a href="DocAjout-update-' . $doc['doc_id'] . '.html"><span class="badge badge-shamrock mb-3 mr-3">Modifier fichier</span></a>';
		...
		$down = '<a target="_blank" href="module/doc/upload/' . $doc['doc'] . '"><span class="badge badge-primary mb-3 mr-3">Téléchargement</span></a>';
	...
	$bdd = null;   */
      ...
      $message = 'Échec id';
		...
		$query = $bdd->prepare("SELECT * FROM energis_doc WHERE doc_id = :doc_id");
    ...
    if ($id == ''){
      ...
      $message = 'Échec id';
		...
		$query = $bdd->prepare("UPDATE energis_doc SET doc_nom = :doc_nom WHERE doc_id = :doc_id");
		$query->bindParam(":doc_id", $id, PDO::PARAM_INT);
	...
	if ($id == ''){
      ...
      $message = 'Échec id';
		...
		$query_del = $bdd->prepare("DELETE FROM energis_doc WHERE doc_id = :id");	
