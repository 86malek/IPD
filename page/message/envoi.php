...
$bdd = new PDO('mysql:host=127.0.0.1;dbname=database', 'root', '');
   if(isset($_POST['envoi_message'])) {
      if(isset($_POST['destinataire'],$_POST['message'],$_POST['objet']) AND !empty($_POST['destinataire']) AND !empty($_POST['message']) AND !empty($_POST['objet'])) {
         ...
         $id_destinataire = $bdd->prepare('SELECT id FROM users WHERE full_name = ?');
         ...
         if($dest_exist == 1) {
            ...
            $ins->execute(array($_SESSION['user_id'],$id_destinataire,$message,$objet));
   ...
   $destinataires = $bdd->query('SELECT full_name FROM users ORDER BY full_name');
