...
if ($job != ''){  
  ...
  }elseif ($job == 'add_new_message'){ 
  
	try 
	{		
    $query_insert = $bdd->prepare("INSERT INTO messages_prive (message_id_expediteur,message_id_destinataire,message,message_objet,dossier_message_id,message_date_crea) VALUES (:user,:destinataire,:message,:objet,:dossier,now())");	
	$query_insert->bindParam(":user", $_GET['user'], PDO::PARAM_INT);
	$query_insert->bindParam(":destinataire", $_GET['destinataire'], PDO::PARAM_INT);
	$query_insert->bindParam(":message", $_GET['message'], PDO::PARAM_STR);
	$query_insert->bindParam(":objet", $_GET['objet'], PDO::PARAM_STR);
	$query_insert->bindParam(":dossier", $_GET['dossier'], PDO::PARAM_INT);	
