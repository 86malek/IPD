...
$(document).ready(function(){
  ...
  var table_cat_fichier = $('#table_cat_fichier_acide').dataTable({
  	"bStateSave": true,
    ...
    "ajax": "table/data_cat_fichier_acide.php?job=get_cat_fichier",
    "columns": [
	  { "data": "fichier",   "sClass": "company_name" },
	  { "data": "statut",   "sClass": "company_name" },
	  { "data": "total",   "sClass": "company_name" },
	  { "data": "traite",   "sClass": "company_name" },
	  { "data": "pourcent",   "sClass": "company_name" },
	  { "data": "collab",   "sClass": "company_name" },
	  { "data": "debut",   "sClass": "company_name" },
	  { "data": "fin",   "sClass": "company_name" },
	  { "data": "temps",   "sClass": "company_name" },
    ...
    "oLanguage": {
      ...
      "sLengthMenu":    "Entrée par page : _MENU_",
      "sInfo":          "Total de _TOTAL_ Entrée (Affichage _START_ à _END_)",
	  "sSearch":          "Recherche",
      "sInfoFiltered":  "(Filtré depuis _MAX_ total Entrée)",
  ...
  $(document).on('click', '#function_edit_cat_fichier', function(e){
    ...
    request.done(function(output){
      if (output.result == 'success'){
        $('.lightbox_content_cat_fichier h2').text('Modification de la catégorie');
        $('#form_cat_fichier button').text('Enregistrement');
  ...
  $(document).on('submit', '#form_cat_fichier.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_cat_fichier').serialize();
      var request   = $.ajax({
        url:          'table/data_cat_fichier_acide.php?job=edit_cat_fichier&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
			  ...
			  sayMyName: {
				...
				action: function () {
				  ...
				  if (input_ligne.val() == '' || input_heure.val() == '') {
					...
					var request = $.ajax({
					url:          'data/objectif-table.php?nbheure=' + input_heure.val() + '&nblignes=' + input_ligne.val() + '&section=' + input_section.val(),
					cache:        false,
					dataType:     'json',
					contentType:  'application/json; charset=utf-8',
					type:         'get'
