$(document).ready(function(){
  ...
  var table_cat_fichier = $('#table_cat_fichier').dataTable({
    "ajax": "table/data_cat_fichier.php?job=get_cat_fichier",
    "columns": [
      { "data": "nom",   "sClass": "nom" },
  ...
  $(document).on('submit', '#form_cat_fichier.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'table/data_cat_fichier.php?job=add_cat_fichier',
        cache:        false,
        data:         form_data,
        dataType:     'json',
  ...
  $(document).on('click', 'a#function_edit_cat_fichier', function(e){
    e.preventDefault();
    show_loading_message();
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'table/data_cat_fichier.php?job=get_cat_fichier_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('submit', '#form_cat_fichier.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_cat_fichier').serialize();
      var request   = $.ajax({
        url:          'table/data_cat_fichier.php?job=edit_cat_fichier&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', 'a#del', function(e){
    ...
    if (confirm("Confirmation de supprission du '" + cat_name + "' !!")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          'table/data_cat_fichier.php?job=delete_cat_fichier&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
