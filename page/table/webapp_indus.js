$(document).ready(function(){
  ...
  var table_companies = $('#table_indus').dataTable({
    "ajax": "table/data_indus.php?job=get_indus",
    "columns": [
      { "data": "Operation" },
  ...
  // Add company button
  $(document).on('click', '#add_indus', function(e){
    e.preventDefault();
    $('.lightbox_content h2').text('Ajouter une opération Industrie');
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'table/data_indus.php?job=add_indus',
        cache:        false,
        data:         form_data,
        dataType:     'json',
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            ...
            show_message("Opération industrie '" + company_name + "' ajoutée avec succés.", 'success');
          }, true);
        } else {
          hide_loading_message();
  ...
  $(document).on('click', '#function_edit_web', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'table/data_indus.php?job=get_indus_add',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
    ...
    request.done(function(output){
      if (output.result == 'success'){
        $('.lightbox_content h2').text('Modification Opération industrie');
        $('#form_company button').text('Enregistrement');
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_company').serialize();
      var request   = $.ajax({
        url:          'table/data_indus.php?job=edit_indus&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', '#del', function(e){
    ...
    if (confirm("Confirmation de supprission du '" + company_name + "' !!")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          'table/data_indus.php?job=delete_indus&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
