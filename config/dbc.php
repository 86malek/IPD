...
define ("ADMIN_LEVEL_BISS", 4);

define ("COORDINATEUR", 4);
...
function checkLead() {
if($_SESSION['team'] == 8 || $_SESSION['team'] == 20 || $_SESSION['team'] == 21) {return 1;} else {return 0;}
}
function checkCollect() {
if($_SESSION['team'] == 16 || $_SESSION['team'] == 20 || $_SESSION['user_id'] == 65) {return 1;} else {return 0;}
}
function checkAuto() {
if($_SESSION['team'] == 7 || $_SESSION['team'] == 20) {return 1;} else {return 0;}
}
function checkQD() {
if($_SESSION['team'] == 2 || $_SESSION['team'] == 20) {return 1;} else {return 0;}
}

function checkSIRET() {
if($_SESSION['team'] == 8 || $_SESSION['team'] == 2) {return 1;} else {return 0;}
}
function checkIE() {
if($_SESSION['team'] == 6) {return 1;} else {return 0;}
}

function checkCNIL() {
if($_SESSION['user_id'] == 82) {return 1;} else {return 0;}
}
/*************** reCAPTCHA KEYS****************/
...
global $db; 	
	...
	if (!isset($_SESSION['user_id']) && !isset($_SESSION['user_name']) ) 
	{
		...
		$rs_ctime = mysqli_query($db,"select `ckey`,`ctime` from `users` where `id` ='$cookie_user_id'") or die(mysqli_error());
		list($ckey,$ctime) = mysqli_fetch_row($rs_ctime);
		 ...
		 if( !empty($ckey) && is_numeric($_COOKIE['user_id']) && isUserID($_COOKIE['user_name']) && $_COOKIE['user_key'] == sha1($ckey)  ) {
				...
				list($user_level, $equipe_id) = mysqli_fetch_row(mysqli_query($db,"select user_level, equipe_id from users where id='$_SESSION[user_id]'"));
...
mysqli_query($db,"UPDATE `users` SET `ckey` = '', `ctime` = '' WHERE `id` = '$sess_user_id' OR `id` = '$cook_user_id'") or die(mysqli_connect_error());
}
...
setcookie("user_name", '', time()-COOKIE_TIME_OUT, '/', null, false, true);
